...
def train_client_models(all_clients, sampled_client_ids, servers: List[Server], drift: Drift,
    ...
    for client in all_clients:
        ...
        if client.client_id in sampled_client_ids:
            ...
            client.fit(server.model.state_dict(), client.client_id, drift_recovery_method, drift.is_drift, drift.drifted_client_indices)
        else:
            # If the client is not sampled, perform local training without server parameters
            client.fit(drift.is_drift, None, client.client_id, drift_recovery_method, drift.drifted_client_indices)
